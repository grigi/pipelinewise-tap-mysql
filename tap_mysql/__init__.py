...
def do_discover(mysql_conn, config):
    discover_catalog(mysql_conn, config.get('filter_dbs'), config.get('filter_tables')).dump()
...
def get_non_binlog_streams(mysql_conn, catalog, config, state):
    ...
    """
    discovered = discover_catalog(mysql_conn, config.get('filter_dbs'), config.get('filter_tables'))
